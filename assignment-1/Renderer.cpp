...
void Renderer::nanogui_init(GLFWwindow *window) {
  ...
  nanogui::ref<nanogui::Window> nanoguiWindow_1 =
      gui_1->add_window(nanogui::Array<int, 2>(0, 0), "Nanogui control bar_1");

  // screen->setPosition(Eigen::Vector2i(-width/2 + 200, -height/2 + 300));

  gui_1->add_group("Camera Position");
  gui_1->add_variable("X", m_camera->position[0])->set_spinnable(true);
  gui_1->add_variable("Y", m_camera->position[1])->set_spinnable(true);
  gui_1->add_variable("Z", m_camera->position[2])->set_spinnable(true);

  gui_1->add_button("Reset Camera", [gui_1]() {
    m_camera->reset();
    gui_1->refresh();
  });

  gui_1->add_group("Camera Rotation");
  gui_1->add_variable("Rotate Angle a", m_animation->local_angle_a)
      ->set_spinnable(true);
  gui_1->add_variable("Rotate Angle b", m_animation->global_angle_b)
      ->set_spinnable(true);

  gui_1->add_button("Rotate Local X", []() { m_animation->rotate_local_x(); });
  gui_1->add_button("Rotate Global Y", []() { m_animation->rotate_global_y(); });
  gui_1->add_button("Reset model", [gui_1]() {
    m_animation->reset();
    gui_1->refresh();
  });
  ...
  for (size_t i = 0; i < obj_list.size(); i++) {
    ...
    if (obj_list[i].obj_name == "main_object") {
      ...
      glUniformMatrix4fv(glGetUniformLocation(shader.program, "model"), 1,
                         GL_FALSE, glm::value_ptr(main_object_model_mat));
    ...
    if (obj_list[i].obj_name == "plane") {
      ...
      glUniformMatrix4fv(glGetUniformLocation(shader.program, "model"), 1,
                         GL_FALSE, glm::value_ptr(plane_model_mat));
      glUniformMatrix4fv(glGetUniformLocation(shader.program, "view"), 1,
                         GL_FALSE, glm::value_ptr(m_camera->get_view_mat()));
      draw_object(shader, obj_list[i]);
    }
    ...
    if (obj_list[i].obj_name == "axis_arrow") {
      ...
      glUniformMatrix4fv(glGetUniformLocation(shader.program, "model"), 1,
                         GL_FALSE, glm::value_ptr(model_mat_x));
      glUniformMatrix4fv(glGetUniformLocation(shader.program, "view"), 1,
                         GL_FALSE, glm::value_ptr(m_camera->get_view_mat()));
      ...
      model_mat_y = glm::rotate(model_mat_y, glm::radians(-90.0f),
                                glm::vec3(0.0f, 1.0f, 0.0f));
      glUniformMatrix4fv(glGetUniformLocation(shader.program, "model"), 1,
                         GL_FALSE, glm::value_ptr(model_mat_y));
      glUniformMatrix4fv(glGetUniformLocation(shader.program, "view"), 1,
                         GL_FALSE, glm::value_ptr(m_camera->get_view_mat()));
      ...
      model_mat_z = glm::rotate(model_mat_z, glm::radians(90.0f),
                                glm::vec3(0.0f, 0.0f, 1.0f));
      glUniformMatrix4fv(glGetUniformLocation(shader.program, "model"), 1,
                         GL_FALSE, glm::value_ptr(model_mat_z));
      glUniformMatrix4fv(glGetUniformLocation(shader.program, "view"), 1,
                         GL_FALSE, glm::value_ptr(m_camera->get_view_mat()));
...
void Renderer::setup_uniform_values(Shader &shader) {
  ...
  glUniformMatrix4fv(glGetUniformLocation(shader.program, "projection"), 1,
                     GL_FALSE, glm::value_ptr(m_camera->get_projection_mat()));
