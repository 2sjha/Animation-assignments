...
void Renderer::nanogui_init(GLFWwindow* window)
{
	...
	nanogui::ref<nanogui::Window> nanoguiWindow_1 = gui_1->add_window(nanogui::Array<int, 2>(0, 0), "sxj220028 Assignment 5");
	...
	auto x1 = gui_1->add_variable("Rotate X1", m_bone_animation->rotation_degree_vector[1][0]);
	auto y1 = gui_1->add_variable("Rotate Y1", m_bone_animation->rotation_degree_vector[1][1]);
	auto z1 = gui_1->add_variable("Rotate Z1", m_bone_animation->rotation_degree_vector[1][2]);

	auto x2 = gui_1->add_variable("Rotate X2", m_bone_animation->rotation_degree_vector[2][0]);
	auto y2 = gui_1->add_variable("Rotate Y2", m_bone_animation->rotation_degree_vector[2][1]);
	auto z2 = gui_1->add_variable("Rotate Z2", m_bone_animation->rotation_degree_vector[2][2]);

	auto x3 = gui_1->add_variable("Rotate X3", m_bone_animation->rotation_degree_vector[3][0]);
	auto y3 = gui_1->add_variable("Rotate Y3", m_bone_animation->rotation_degree_vector[3][1]);
	auto z3 = gui_1->add_variable("Rotate Z3", m_bone_animation->rotation_degree_vector[3][2]);
	gui_1->add_button("Reset Bone Rotation", [x1, y1, z1, x2, y2, z2, x3, y3, z3]() {
...
{
	// 4 bones (including the root bone)
	for (int j = 0; j < 4; ++j) {
		Object *bone_obj = nullptr;
		for (unsigned int i = 0; i < obj_list.size(); i++)
		{
			// identify each bone by their name
		...
		bone_obj_mat = glm::scale(m_bone_animation->final_mat[j], m_bone_animation->scale_vector[j]);
		glUniformMatrix4fv(glGetUniformLocation(shader.program, "model"), 1, GL_FALSE, glm::value_ptr(bone_obj_mat));
